...
- 技术博客友好的封面策略：只展示作者明确设置的封面；无封面文章自动使用标题与摘要卡片，不请求第三方随机图片。
...
2. 下载 `theme-sky-weather-v1.0.27.zip`。
3. 进入 Halo 后台。
4. 打开「外观」->「主题」->「安装主题」。
5. 上传下载的 zip 文件并启用主题。
...
- 作者为文章设置封面时，现代、媒体和杂志列表继续展示该封面。
- 文章没有封面时，列表自动展示标题、摘要、分类、日期和阅读统计。
- 主题不再内置或请求第三方随机图片 API，也不会为了提取首图而在列表页解析整篇正文。
- 从旧版本升级后，即使 Halo ConfigMap 中仍保留旧的随机图片地址，文章列表也会忽略该配置。

## 文档页说明

主题提供 Docsme 文档页适配，包含文档列表、详情页、目录、长文档渐进加载和代码块样式修复。`v1.0.24` 修复了文档页代码块被全局 `code` 样式压成单行、长文档滚动懒加载后 Shiki 未重新高亮的问题。`v1.0.25` 修复了 `v1.0.24` 包中部分模板文件编码损坏导致的页面解析失败问题。`v1.0.26` 将布局模板中的资源版本号改为跟随主题版本，修复旧版 `doc.js` 缓存导致超长文档懒加载代码块保持模糊的问题。
...
2. 推送代码到 `main`。
3. 创建版本 tag 并推送。
4. GitHub Actions 会自动创建 Release，并上传 `theme-sky-weather-v1.0.27.zip`。
